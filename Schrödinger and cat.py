...
outface=pygame.image.load("outface.png")
#创建地图对象
bg1 = MyMap(0,0)
bg2 = MyMap(300,0)
#创建一个精灵组
...
#创建怪物精灵
dragon = MySprite()
dragon.load("dragon.png", 159, 163, 2)
dragon.position =200, 200
group.add(dragon)
...
#创建子弹精灵
arrow = MySprite()
arrow.load("flame.png", 40, 40, 1)
arrow.position = 800,500
group.add(arrow)
    ...
    if button.game_start == True:
        if game_pause :
            ...
            for i in range(0,200):
                element = MySprite()
                element.load("fruit.bmp", 75, 41, 1)
